...
class BayesianElasticityModel:
    ...
    def predict_demand(self, prices):
        """
        Predict demand for SKUs given new prices.
        
        Parameters:
        -----------
        prices : array-like
            Array of new prices corresponding to the rows in the model's data
        ...
        if len(prices) != len(self.data):
            raise ValueError("Length of prices must match the number of rows in the training data")
        
        # Create a copy of the data and update prices
        new_data = self.data.copy()
        new_data[self.price_col] = prices
        
        # Generate predictions using the existing predict method
        predictions = self.predict(new_data=new_data)
        
        # Return the mean predicted demand in original scale
        return predictions['pred_demand_mean'].values
...
class BayesianElasticityModel:
    ...
    def generate_dynamic_dummy_data(
        n_categories=3,
        n_brands_per_category=2,
        n_skus_per_brand=3,
        start_date="2023-01-01",
        ...
        """
        Generates synthetic data with dynamic SKU costs and max inventory, 
        simulating real-world retail scenarios.
        ...
        lifecycle_stages : list
            List of lifecycle stage names.
        lead_time_days : int
            Lead time in days for inventory restocking.
        seed : int
            Random seed for reproducibility.
        ...
        --------
        pd.DataFrame
            Synthetic dataset with dynamic SKU costs and max inventory.
        """
        np.random.seed(seed)
        date_range = pd.date_range(start=start_date, end=end_date, freq="D")
        n_days = len(date_range)
        ...
        holiday_schedule = [(12, 25), (7, 4), (1, 1)]
    
        # Lifecycle stage cost multipliers
        lifecycle_cost_multipliers = {
            "npi": 1.2,
            "mature": 0.9,
            "eol": 1.1,
        }
        ...
        for cat_idx in range(n_categories):
            cat_price_factor = 0.8 + 0.4 * np.random.random()
    
            for brand_idx in range(n_brands_per_category):
                brand_price_factor = 0.9 + 0.2 * np.random.random()
                global_brand_idx = cat_idx * n_brands_per_category + brand_idx
    
                for sku_idx in range(n_skus_per_brand):
                    global_sku_idx = global_brand_idx * n_skus_per_brand + sku_idx
                    sku_base_price = 100 * cat_price_factor * brand_price_factor * (
                        0.95 + 0.1 * np.random.random()
                    )
                    base_cost = sku_base_price * 0.5  # Base cost as a fraction of base price
    
                    # Demand parameters
                    k = 6000 * (0.8 + 0.4 * np.random.random())
                    price_elasticity = -1.5 - np.random.random()
    
                    # Historical demand for inventory calculation
                    historical_demand = [k] * lead_time_days  # Initialize with base demand
                    ...
                    for day_idx, date in enumerate(date_range):
                        ...
                        # Dynamic SKU Cost Calculation
                        stage_cost_multiplier = lifecycle_cost_multipliers[current_stage]
                        seasonal_cost_adjustment = 1 + 0.1 * np.sin(2 * np.pi * day_idx / 365)
    
                        # Holiday cost reduction
                        holiday_cost_reduction = 0
                        if (date.month, date.day) in holiday_schedule:
                            holiday_cost_reduction = 0.1  # 10% reduction
    
                        sku_cost = base_cost * stage_cost_multiplier * seasonal_cost_adjustment * (1 - holiday_cost_reduction)
                        sku_cost = max(0, sku_cost)
    
                        # Price calculation (as in the original function)
                        ...
                        seasonality = 1.0 + 0.05 * np.sin(2 * np.pi * day_idx / 365)
                        weekly_pattern = 1.0 + 0.02 * np.sin(2 * np.pi * date.dayofweek / 7)
                        price_noise = 1.0 + 0.03 * (np.random.random() - 0.5)
                        price = sku_base_price * price_multiplier * seasonality * weekly_pattern * price_noise
    
                        comp_price = price * (0.9 + 0.2 * np.random.random())
    
                        # Demand calculation
                        k = 6000 * (0.8 + 0.4 * np.random.random())
                        base_demand = k * ((price / sku_base_price) ** price_elasticity)
                        trend = 1.0 + 0.1 * day_idx / n_days
                        demand_seasonality = 1.0 + 0.2 * np.sin(2 * np.pi * day_idx / 365)
                        demand_noise = 1.0 + 0.1 * (np.random.random() - 0.5)
    
                        lifecycle_demand_factor = 1.0
                        if current_stage == "npi":
                            lifecycle_demand_factor = 0.7
                        elif current_stage == "mature":
                            lifecycle_demand_factor = 1.2
                        else:
                            lifecycle_demand_factor = 0.8
    
                        demand = (
                            base_demand * trend * demand_seasonality * demand_noise * lifecycle_demand_factor
                        )
    
                        # Dynamic Max Inventory Calculation
                        average_past_demand = np.mean(historical_demand)
                        safety_stock_factor = 1.2  # Safety stock to account for demand variability
                        holiday_inventory_boost = 1.0
                        if (date.month, date.day) in holiday_schedule:
                            holiday_inventory_boost = 1.5  # Increase inventory by 50% before holidays
    
                        max_inventory = average_past_demand * safety_stock_factor * holiday_inventory_boost
                        max_inventory = max(0, max_inventory)
    
                        # Update historical demand
                        historical_demand.pop(0)
                        ...
                        data.append(
                            {
                                ...
                                "demand": demand,
                                "sku_cost": sku_cost,
                                "sku_max_inventory": max_inventory,
                            }
                        )
